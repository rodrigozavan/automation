...
class ConfigDriver:
    ...
    def set_driver(self, name='chrome'):
        try:
            ...
            if name == 'firefox':
                self._driver = webdriver.Firefox(executable_path=config.Firefox, options=self._firefox_options)
            else:
                self._driver = webdriver.Chrome(executable_path=config.Chrome, options=self._chrome_options)
